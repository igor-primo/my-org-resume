...
Minhas realizações:
- Instalação de um cluster Kubernetes utilizando Ansible.
- Instalação e configuração de instâncias de softwares para homologação no novo ambiente Kubernetes, como
  Gitlab, Grafana e Prometheus.
- Provisionamento automatizado de servidores através de Terraform em um nó Proxmox on-premises.
- Elaboração de documentação dos processos de instalação e dos arquivos de configuração desses softwares
...
- Desenvolveu uma API RESTful em Node.js como componente para uma aplicação de fomento ao consumo consciente.
- Configurou banco de dados PostgreSQL e gerenciou dados da aplicação, realizando migrações e modificações adequadas quando necessárias.
- Realizou análise de requisitos e de conformidade do software para com os seus requisitos.
